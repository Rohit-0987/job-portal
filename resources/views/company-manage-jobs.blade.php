											...
											<tbody>
												...
												<tr>
													<td><div class="dash-title">
														<h4 class="mb-0 ft-medium fs-sm">{{$row->job_title}}
															{{-- <span class="medium theme-cl rounded text-success bg-light-success ml-1 py-1 px-2">Pending</span> --}}
														</h4>
														</div>
													</td>
													<td>{{$row->carrer_level}}</td>
													<td>{{$row->created_at->format('d M Y')}}</td>
													<td>{{date('d M Y', strtotime($row->deadline))}}</td>
													<td><a href="company-manage-application-filter{{$row->jobs_id}}" class="theme-bg text-light rounded px-3 py-2 ft-medium">Total @if(empty($row->getJobApplications)) 0 @else  {{$row->getJobApplications->count()}} @endif</a></td>
													<td>
														<div class="dash-action">
															<a href="company-manage-jobs-edit{{$row->jobs_id}}" class="p-2 circle text-info bg-light-info d-inline-flex align-items-center justify-content-center mr-1"><i class="lni lni-eye"></i></a>
